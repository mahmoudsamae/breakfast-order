...
export const STAFF_ORDER_SELECT =
  "id,order_number,customer_name,created_at,status,total_amount,pickup_date,paid_at,delivered_at,not_picked_up_reason,not_picked_up_note,staff_hidden_at,order_items(id,product_id,menu_id,quantity,unit_price,products(name,category,image_url),menus(id,name,description,image_url,menu_items(product_id,quantity,products(name,category,image_url))))";

/** Same as STAFF_ORDER_SELECT before paid_at migration (graceful fallback). */
export const STAFF_ORDER_SELECT_WITHOUT_PAID_AT =
  "id,order_number,customer_name,created_at,status,total_amount,pickup_date,delivered_at,not_picked_up_reason,not_picked_up_note,staff_hidden_at,order_items(id,product_id,menu_id,quantity,unit_price,products(name,category,image_url),menus(id,name,description,image_url,menu_items(product_id,quantity,products(name,category,image_url))))";

export function isMissingPaidAtColumn(error) {
  if (!error) return false;
  if (error.code === "42703") return String(error.message || "").includes("paid_at");
  return String(error.message || "").includes("paid_at");
}
