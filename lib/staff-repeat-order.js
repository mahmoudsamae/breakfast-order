...
export async function createStaffRepeatOrder(supabase, { branchId, sourceOrderId, lines, pickupDate }) {
  ...
  const repeatable = ["pending", "delivered", "not_picked_up"];
  if (!repeatable.includes(source.status)) {
    const err = new Error("Diese Bestellung kann nicht für morgen wiederholt werden.");
    err.status = 400;
    throw err;
  }
